...
(* CHANGE add shifts for expressions of the form Shift *)
...
fun shift_exp (deltax, deltay, exp, env) = 
	case exp of 
		NoPoints => exp
		| Point(a, b) => Point(a + deltax, b + deltay)
		| Line(m, b) => Line(m, b + deltay - m * deltax)
		| VerticalLine(x) => VerticalLine(x + deltax)
		| LineSegment(x1,y1, x2, y2) =>
			let 
				val Point(x1n, y1n) = shift_exp (deltax, deltay,Point(x1, y1), env)
				val Point(x2n, y2n) = shift_exp (deltax, deltay,Point(x2, y2), env)
			in
				LineSegment(x1n,y1n,x2n,y2n)
			end
...
(* CHANGE: Add a case for Shift expressions *)
	  | Shift(deltax, deltay, exp) => shift_exp (deltax, deltay, eval_prog(exp, env), env)
